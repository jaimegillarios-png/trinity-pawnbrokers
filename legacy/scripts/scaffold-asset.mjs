#!/usr/bin/env node
/**
 * Scaffolds a new asset page's content file with placeholder copy.
 *
 *   node scripts/scaffold-asset.mjs <slug> "<Display name>" <singular> <plural> [hero-image]
 *   node scripts/scaffold-asset.mjs gold "Gold" "gold item" "gold"  images/v2/gold.jpg
 *
 * Writes src/content/<slug>.js (never overwrites an existing file), then run
 * scripts/build-asset-pages.mjs to produce <slug>.html.
 *
 * Every string a copywriter must replace is marked TODO — the build script
 * counts them so you can see at a glance what is still placeholder.
 */
import { writeFile, access } from 'node:fs/promises';
import { fileURLToPath } from 'node:url';
import { dirname, resolve } from 'node:path';

const [slug, display, singular, plural, hero] = process.argv.slice(2);
if (!slug || !display) {
  console.error('usage: scaffold-asset.mjs <slug> "<Display name>" [singular] [plural] [hero-image]');
  process.exit(1);
}
const one = singular || display.toLowerCase();
const many = plural || `${one}s`;
const img = hero || `images/${slug}/hero.jpg`;

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const out = resolve(root, `src/content/${slug}.js`);
if (await access(out).then(() => true).catch(() => false)) {
  console.error(`✗ src/content/${slug}.js already exists — refusing to overwrite.`);
  process.exit(1);
}

const file = `/* Trinity — asset page content: ${display.toUpperCase()}
 *
 ...
 * After editing, run:  node scripts/build-asset-pages.mjs ${slug}
 */
export default {
  slug: '${slug}',
  noun: { singular: '${one}', plural: '${many}' },

  meta: {
    title: 'Pawn loans against ${many} — Trinity Pawnbrokers',
    description: 'TODO — one-sentence description for search results, mentioning ${many}, £500 minimum and the City of London.',
  },

  hero: {
    image: { src: '${img}', alt: 'TODO — describe the ${one} image' },
    eyebrow: 'Pawnbroker loans against ${many} · City of London · Est. 2013',
  ...
  form: {
    ...
    backLabel: '← Back to your ${one}',
    noteStep1: 'Free, no obligation, same-day offer, and nothing recorded on your credit file.',
    noteStep2: 'Free, no obligation, same-day offer, and nothing recorded on your credit file. We’ll never ask you to send your ${one} before you have an offer you’re happy with.',
  },
  ...
  lendAgainst: {
    ...
    heading: 'The ${many} we lend against',
  ...
  borrow: {
    ...
    intro: 'TODO — how ${many} are valued, and why that beats the high street.',
    specs: [
    ...
    example: {
      ...
      statement: 'TODO — a £X,XXX loan against a specific ${one}, over 6 months',
      rows: [
      ...
      note: 'Illustrative only. Your own figures depend on the specific ${one}, its condition, and the market on the day — you see them in full before you commit to anything.',
    },
  },

  how: {
    eyebrow: 'How it works',
    heading: 'TODO — four steps, ending with the ${one} back with you.',
    steps: [
      { icon: 'ph-chat-teardrop-text', title: 'Tell us about your ${one}', body: 'TODO' },
  ...
  why: {
    ...
    heading: 'A pawnbroker for ${many} you intend to <span>keep</span>.',
    intro: 'TODO — we lend against it, we do not buy it.',
    rows: [
      { icon: 'ph-diamond', label: 'Your ${one}', trinity: 'Yours throughout — a loan, never a sale', highStreet: 'Often bought outright' },
  ...
  faqs: {
    eyebrow: '${display} FAQs',
    heading: 'Asked, answered',
    items: [
      { q: 'Do I have to sell my ${one}?', a: 'No. This is a loan against it. You keep ownership throughout and take it back when you repay.' },
      { q: 'TODO — an objection specific to ${many}', a: 'TODO' },
  ...
  closing: {
    eyebrow: 'Get started',
    heading: 'Find out what your ${one} is worth',
    intro: 'A confidential valuation, at no cost, with no obligation to proceed.',
    cta: { label: 'Value my ${one}', href: '#value-form' },
    contactPrefix: 'Or speak to a specialist:',
    contactSuffix: '· WhatsApp',
  },

  specimenBar: 'Asset template · ${display} instance · PLACEHOLDER COPY — not for review',
};
`;

await writeFile(out, file, 'utf8');
console.log(`✓ src/content/${slug}.js  (${(file.match(/TODO/g) || []).length} TODOs)`);
console.log(`  next: node scripts/build-asset-pages.mjs ${slug}`);
